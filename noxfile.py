...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/snap/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/snap/*")
            ...
            #    "--include=src/saltext/snap/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/snap/config/schemas",
    )
